...
export default {
  ...
  computed: {
    ...mapGetters(["showBrgys", "requestReport"]),
  },
  async mounted() {
  ...
  methods: {
    generate() {
      ...
      setTimeout(() => {
        this.$store.dispatch("generateReport", requestParams);
    ...
    savedPDF() {
      ...
      const rawData = this.$store.getters["requestReport"];
      let body = [...rawData.map((data) => [...Object.values(data)])];

      doc.text("Request Report", 10, 20);
      doc.autoTable({
        margin: { top: 30 },
